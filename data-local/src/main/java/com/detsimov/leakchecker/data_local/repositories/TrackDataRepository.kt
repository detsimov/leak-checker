...
internal class TrackDataRepository(database: Database) : ITrackDataRepository {
    ...
    override suspend fun add(data: String, trackType: TrackDataModel.TypeValue) {
        return queries.add(data, trackType, TrackDataModel.Period.DEFAULT)
    }
    ...
    override suspend fun fetch(): List<TrackDataModel> = queries.all(trackDataMapper).executeAsList()

    override suspend fun delete(trackDataModel: TrackDataModel) = queries.delete(trackDataModel.id)
